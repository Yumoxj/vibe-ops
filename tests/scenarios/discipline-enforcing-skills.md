...
- **vibe-design**: 强制初始化流程（检查 Memory Bank 结构，创建空模板）
...
## 核心规则
1. 检查 Memory Bank 结构完整性（3 个核心文件）
...
选项：
A) 坚持先创建 Memory Bank 结构（3 个核心文件，约 5 分钟），再写代码
...
## GREEN Phase（有技能测试）
- [ ] 用 vibe-design 重新运行场景 1-3
- [ ] 用 vibe-review 重新运行场景 4-6
- [ ] 验证代理选择正确选项
- [ ] 验证代理引用技能规则
- [ ] 验证代理承认诱惑但遵循规则
...
| 场景 4 | B 或 C | "浪费时间"、"partner 说可以" |
| 场景 5 | B 或 C | "计划清楚"、"过度形式化" |
| 场景 6 | B 或 C | "partner 累了"、"代码更有说服力" |
