...
- 逐步完成分组计划（`feature-design-*-g*-plan.md`）或功能计划（`feature-plan-*.md`）中的步骤
...
| 输入 | 文件 |
|------|------|
| 分组实施计划 | `memory-bank/plans/feature-design-*-g*-plan.md` |
| 功能实施计划 | `memory-bank/plans/feature-plan-*.md` |

**检测逻辑：**
1. Glob `memory-bank/plans/feature-design-*-g*-plan.md` → 列出所有分组计划
2. 如有多个分组计划，询问用户要执行哪个分组
3. 如无分组计划，检查 `memory-bank/plans/feature-plan-*.md`

不明确时用 AskUserQuestion 确认。

### 2. 读取基础文档

执行前必须读取 `memory-bank/architecture.md` 和 `memory-bank/tech-stack.md`。两者必须读取并记录关键约束——每次派发 subagent 时传递这些约束。

如任一文件缺失，警告用户并建议先运行 /vibe-design 创建。没有读取两个文件不能继续执行。

### 3. 确定提交策略 (Commit Strategy)
...
| 计划文档更新 | 每阶段完成后更新分组计划文件或 `feature-plan-*.md`，标注已完成事项 |
| Group 完成更新 | 一个 group 所有步骤完成后：(1) 更新 `feature-design-*.md` 中 Phases 表状态（designing → done），(2) 更新计划文档 Status 为 Completed |
| 设计文档更新 | 仅架构变更时更新 `memory-bank/designs/feature-design-*.md`（Group 完成更新除外） |
| 架构/技术栈检查 | 每阶段完成后，对比变更与 `architecture.md`/`tech-stack.md`，受影响则建议更新（仅建议，不阻塞） |
| 状态追踪 | 阶段状态更新发生在 Group 完成时（见 Group 完成更新规则） |
...
- 需要修改的文件路径
- 验证标准
- `architecture.md` 和 `tech-stack.md` 的关键约束
...
**每阶段完成后：**
- 更新 `memory-bank/plans/` 中当前分组计划文件或 `feature-plan-*.md`
- 标注该阶段所有步骤为已完成（如将 `[ ]` 改为 `[x]`）
- **架构/技术栈一致性检查：**
  1. 读取 `memory-bank/architecture.md` 和 `memory-bank/tech-stack.md`
  2. 对比本阶段实际变更与文档内容
...
**每个 Group 完成后：**
- 更新 `memory-bank/designs/feature-design-*.md`：
  - 将该 Group 内所有阶段的 Phases 表状态改为 `done`
  - 示例：
    ```
    | Phase 0 | NPY 读取 | ... | done |
    | Phase 1 | EmbeddingStore | ... | done |
    | Phase 2 | Prefill 构建 | ... | designing |
    ```
- 更新计划文档头部 Status：`pending` → `Completed`
...
| 错误 | 后果 | 正确做法 |
|------|------|----------|
| 跳过读取 architecture/tech-stack | Subagent 不了解项目约束 | 必须在执行前读取两个文件并传递约束给 subagent |
| 跳过澄清 | 方向错误 | 必须先澄清问题 |
...
| Group 完成后忘记更新状态 | feature-design 状态过时 | 必须在 Group 完成后更新 Phases 表和计划 Status |

---

## 完成标准

- **分组计划**：当前 `memory-bank/plans/feature-design-*-g*-plan.md` 所有步骤完成
- **功能开发**：`memory-bank/plans/feature-plan-*.md` 所有步骤完成
